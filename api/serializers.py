...
class WishSerializer(serializers.ModelSerializer):
    ...
    dontated = serializers.DecimalField(max_digits=5, decimal_places=2, source='get_transaction_sum', read_only=True)
    days_left = serializers.CharField(source='get_timeleft', read_only=True)
    api_url = serializers.CharField(read_only=True)

    class Meta:
        model = Wish
        fields = ('id', 'title', 'description', 'price', 'dontated', 'days_left', 'status', 'writers', 'api_url')